...
function parseTable(table, pageNumber) {
  ...
  if (!hasHeaderSignal) {
    return { items: [], mapping: null, headers };
  }
...
export function dedupeFurnitureItems(items) {
  const bySlug = new Map();
  for (const item of items) {
    const slug = item.slug || slugify(item.title);
    const existing = bySlug.get(slug);
    if (!existing) {
      bySlug.set(slug, { ...item, slug, matchedImages: [...(item.matchedImages ?? [])] });
      continue;
    }
    bySlug.set(slug, {
      ...existing,
      category: existing.category ?? item.category,
      dimensions: existing.dimensions ?? item.dimensions,
      dimensionDisplay: existing.dimensionDisplay ?? item.dimensionDisplay,
      materials: existing.materials ?? item.materials,
      description: existing.description ?? item.description,
      width: existing.width ?? item.width,
      depth: existing.depth ?? item.depth,
      height: existing.height ?? item.height,
      missing: [...new Set([...(existing.missing ?? []), ...(item.missing ?? [])])],
      matchedImages: [...existing.matchedImages, ...(item.matchedImages ?? [])].filter(
        (image, index, list) =>
          list.findIndex((entry) => entry.pageNumber === image.pageNumber && entry.index === image.index) === index
      )
    });
  }
...
export function sanitizePdfReportForClient(report) {
  const items = (report.items ?? []).map((item) => ({
    title: item.title,
    slug: item.slug,
    category: item.category,
    dimensions: item.dimensionDisplay ?? item.dimensions,
    materials: item.materials,
    description: item.description,
    pageNumber: item.pageNumber,
    source: item.source,
    imageCount: item.matchedImages?.length ?? 0,
  ...
  return {
    ...
    furnitureItemCount: report.furnitureItemCount,
    embeddedImageCount: report.embeddedImageCount,
    imageMatchedItemCount: report.imageMatchedItemCount,
    ...
    rowsMissingDataCount: report.rowsMissingDataCount,
...
export async function analyzeFurniturePdf(source, options = {}) {
  ...
  const dedupedItems = dedupeFurnitureItems(allItems);
  const { matchedCount, unmatchedPages } = matchImagesToItems(dedupedItems, embeddedImages);
  ...
  const imageBuffers = {};
  if (options.includeImageBuffers) {
    for (const page of imageResult.pages ?? []) {
      for (let index = 0; index < (page.images ?? []).length; index += 1) {
        const image = page.images[index];
        if (image?.imageBuffer?.length) {
          imageBuffers[`${page.pageNumber}:${index}`] = image.imageBuffer;
        }
      }
    }
  }
  ...
  const categories = [...new Set(dedupedItems.map((item) => item.category).filter(Boolean))].sort();
  const materials = [...new Set(dedupedItems.map((item) => item.materials).filter(Boolean))].sort();
  const dimensions = [...new Set(dedupedItems.map((item) => item.dimensionDisplay).filter(Boolean))].sort();

  const rowsMissingData = dedupedItems.filter((item) => item.missing.length > 0);
  ...
  const imageMappingStatus =
    embeddedImages.length === 0
      ...
      : matchedCount === 0
        ...
        : `${matchedCount} of ${dedupedItems.length} item(s) have images on the same page (${embeddedImages.length} total image(s)).`;
  ...
  return {
    ...
    furnitureItemCount: dedupedItems.length,
    ...
    materials,
    dimensions,
    examples: dedupedItems.slice(0, 10),
    ...
    savedImages,
    imageBuffers: options.includeImageBuffers ? imageBuffers : undefined,
